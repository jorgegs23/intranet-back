...
//			+ " (:#{#filtro.mes} IS NULL OR :#{#filtro.mes} = e.competicion.competicion) AND "
			...
			+ " (:#{#filtro.usuario} IS NULL OR	"
				...
				+ " :#{#filtro.usuario} = e.oficial3.id OR :#{#filtro.usuario} = e.oficial4.id "
			+ ") ")
	List<DesignacionEntity> filterList(@Param("filtro") DesignacionFiltroDto filtro);
	...
	@Query("SELECT e FROM DesignacionEntity e WHERE e.id IN :idsDesignaciones")
	List<DesignacionEntity> filterListByIds(@Param("idsDesignaciones") List<Integer> idsDesignaciones);
}
